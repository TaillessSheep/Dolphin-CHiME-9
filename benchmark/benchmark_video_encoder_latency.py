...
def run_benchmark(video_encoder, device, warmup: int, repeat: int):
    video_encoder = video_encoder.to(device).eval()

    # 1-second mouth ROI: 25 FPS, grayscale, 88x88
    # Shape follows Dolphin Inference.py: [B, C, T, H, W]
    x = torch.randn(1, 1, 25, 88, 88, device=device)

    def forward_once():
        return video_encoder(x)
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark Dolphin DP-LipCoder / video encoder 1-second forward latency."
    )
    parser.add_argument(
        "--dolphin-root",
        default=None,
    ...
    print("=" * 80)
    print("Benchmark 2: Dolphin DP-LipCoder / VideoEncoder forward latency")
    print("=" * 80)
    ...
    print("Output visual feature shape expected: [1, 25, 3872]")
    print("Batch size:   1")
    print("GPU warmup:", args.warmup)
    ...
    if not args.cpu_only and torch.cuda.is_available():
        ...
        video_encoder_gpu = model_gpu.video_encoder
        ...
        print("\nGPU video encoder latency:")
        print(gpu_result)

    if not args.gpu_only:
        model_cpu = Dolphin.from_pretrained(args.model_source).eval()
        video_encoder_cpu = model_cpu.video_encoder
        ...
        print("\nCPU video encoder latency:")
        print(cpu_result)

    print("\nPaper Table 1 DP-LipCoder reported latency:")
    print("GPU latency: 23.24 ms")
